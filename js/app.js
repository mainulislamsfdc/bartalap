...
class App {
  ...
  setupEventListeners() {
    ...
    // In app.js, update the speechResult event handler:

    window.addEventListener("speechResult", async (event) => {
      const { transcript, isFinal } = event.detail;

      console.log("Speech result received:", { transcript, isFinal });

      // Show the original text immediately
      ...
      if (isFinal) {
        try {
          ...
          // Update UI with both original and translation
          ...
          this.uiController.showError(
            "Error during translation. Please try again."
          );
        } finally {
