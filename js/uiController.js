...
export default class UIController {
  ...
  updateUI(state) {
    ...
    if (this.recordingStatus) {
      this.recordingStatus.textContent = state.isRecording
        ? "Recording..."
        : "";
    }
  }
  ...
  initializeVoicePreference() {
    document.querySelectorAll(".voice-btn").forEach((btn) => {
      btn.classList.toggle(
        "active",
        btn.dataset.voice === this.preferredVoiceGender
      );
  ...
  //'Google UK English Male - en-GB' 'Google UK English Female - en-GB'
    speakText(text, lang) {
        ...
        // ❗ No specific voice selection — let browser choose best default for lang

        const timeout = setTimeout(() => {
            console.warn("🛑 Speech onend not triggered — forcing unlock");
            audioStateManager.stopSpeaking();
        }, 10000);
  ...
  updateCurrentText(text) {
    ...
    if (!text) {
      this.currentText.innerHTML =
        '<p class="placeholder">Press the microphone button to start speaking...</p>';
      return;
    }
